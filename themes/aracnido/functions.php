...
function fr_dropdown_chapters(){
			...
			echo '<div class="selector" onclick="toggleMenu(this)">Tomo: '.fr_selected_chapter();
			echo '<div class="options">';
...
function fr_dropdown_chapters_2(){
			if (fr_selected_comic() == "") return;
			echo '<div class="selector2" onclick="toggleMenu(this)">Tomo: '.fr_selected_chapter();
			echo '<div class="options">';
			$temp = fr_available_chapters();
			sort($temp);
            foreach($temp as $key=>$value){
	...
	echo '<div class="theList">';
		foreach($temp as $key=>$value){
			...
			$temp_chapters = fr_available_chapters();
        		...
        		if(!empty($temp_chapters)) echo  "<a href='".fr_get_href($value, max(fr_available_chapters()))."'> <h5 class='ultimo'> Última publicación: ".max(fr_available_chapters())." »</h5></a>";
        		else echo "No hay contenido disponible.";
...
function fr_list_chapters(){
	...
	if(!empty($temp))
	foreach($temp as $key=>$value){
		echo '<div class="chapter"><b>'.$value.'</b><div style="float:right; color:#43c8f0;"><a href="'.fr_get_href(fr_selected_comic(),$value).'">Leer</a></div></div>';
	}
	else echo "No hay contenido disponible.";
	echo '</div>';
}
...
function fr_show_page(){
	...
	if (empty($temp_avail)) 
		{
			echo '<div class="theList">Este tomo no tiene contenido alguno.</div>';
			return;
		}
